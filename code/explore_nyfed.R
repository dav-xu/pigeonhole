...
#   4. Produces proof-of-concept plots: offer distributions by gender,
#      faceted by education x region and education x industry
# ==============================================================================
...
searchers <- data %>%
    ...
    filter(offers <= 20) # cap outliers (unlikely anyone received 100 offers...)
...
plot_edu_region <- searchers %>%
    filter(!is.na(region)) %>%
    ggplot(aes(x = offers, fill = gender)) +
    geom_histogram(alpha = 0.5, position = "identity", bins = 20, color = "white", linewidth = 0.2) +
    facet_grid(rows = vars(edu_label), cols = vars(region), scales = "free_y") +
    scale_fill_brewer(palette = "Set2") +
    theme_minimal(base_family = "Palatino") +
    labs(
        title = "Job offers in last 4 weeks: Men vs. Women",
        subtitle = "By education level and region (NYFED SCE 2013-2021)",
        x = "Number of offers",
        y = "Count",
        fill = "Gender"
    ) +
...
plot_edu_industry <- searchers %>%
    filter(!is.na(industry_macro)) %>%
    ggplot(aes(x = offers, fill = gender)) +
    geom_histogram(alpha = 0.5, position = "identity", bins = 10, color = "white", linewidth = 0.2) +
    facet_grid(rows = vars(edu_label), cols = vars(industry_macro), scales = "free_y") +
    scale_fill_brewer(palette = "Set2") +
    theme_minimal(base_family = "Palatino") +
    labs(
        title = "Job offers in last 4 weeks: Men vs. Women",
        subtitle = "By education level and industry (NYFED SCE 2013-2021)",
        x = "Number of offers",
        y = "Count",
        fill = "Gender"
    ) +
